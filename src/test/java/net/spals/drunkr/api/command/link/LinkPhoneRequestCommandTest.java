...
/**
 * Unit tests for {@link LinkPhoneRequestCommand}.
 *
 * @author spags
 */
public class LinkPhoneRequestCommandTest {
    ...
    private LinkPhoneRequestCommand command;
    ...
    public void setUp() {
        ...
        command = new LinkPhoneRequestCommand(generator, dbService, keyStore, i18nSupport, twilioClient);

        request = ImmutableMap.<String, Object>builder()
            .put("user", DRUNK)
            .put("phoneNumber", DRUNKS_NUMBER)
            .put("defaultRegion", Optional.empty())
            .build();
    ...
    public void duplicatePhoneNumber() {
        when(dbService.getPerson(DRUNKS_NUMBER)).thenReturn(Optional.of(DRUNK));

        final Response response = command.run(request);

        assertThat(response)
            .hasStatus(CONFLICT)
            .hasErrorMessage(i18nSupport.getLabel("command.link.phone.request.duplicate"));
    }
    ...
    public void sendCode() {
        ...
        assertThat(message).isEqualTo(i18nSupport.getLabel("command.link.phone.request", CODE));
